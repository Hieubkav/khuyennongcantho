...
export default function LichSuKhaoSat() {
  ...
  useEffect(() => {
    fetch("/api/member/me").then((r) => r.json()).then((j) => { if (j?.ok) setMe({ sub: j.sub, username: j.username, name: j.name ?? undefined }); }).catch(() => {});
  }, []);
  const [fromDay, setFromDay] = useState(today());
  const [toDay, setToDay] = useState(today());
  const list = useQuery(api.members.surveysInRange as any, me ? { memberId: me.sub as any, fromDay, toDay } as any : "skip" as any);
